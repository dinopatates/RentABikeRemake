    ...
    <Layout>
      <main className="mx-auto grid max-w-6xl items-center gap-12 px-6 pb-20 pt-10 md:grid-cols-2">
        <section>
          ...
          <h1 className="text-4xl font-bold leading-tight md:text-5xl">Your next journey starts here.</h1>
          <p className="mt-5 max-w-md leading-relaxed text-gray-600">Creez votre compte avec les informations demandees par votre profil utilisateur.</p>
        </section>
        <form className="grid gap-5 rounded-2xl border border-gray-200 p-7 shadow-sm" onSubmit={handleSubmit}>
          <div><h2 className="text-2xl font-bold">Register</h2><p className="mt-1 text-sm text-gray-600">Creez votre compte.</p></div>
          <label className="grid gap-2 text-sm font-bold">Name<input className="min-h-12 rounded-lg border border-gray-300 px-4 text-base font-normal outline-none focus:border-[#5534e7] focus:ring-2 focus:ring-[#5534e7]/20" name="name" onChange={updateField} required type="text" value={form.name} /></label>
